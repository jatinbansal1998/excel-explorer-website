  ...
  private static getRecommendations(features: Record<string, boolean>): string[] {
    ...
    if (!features.modern) {
      recommendations.push(
        'Please update to a modern browser (Chrome 60+, Firefox 60+, Safari 12+, Edge 79+) for the best experience.',
      )
    }

    if (Object.values(features).some((f) => !f)) {
      recommendations.push(
        'Some features may be limited in your browser. Consider updating or switching browsers.',
      )
    }
    ...
    if (userAgent.includes('MSIE') || userAgent.includes('Trident')) {
      recommendations.push(
        'Internet Explorer is not supported. Please use a modern browser like Chrome, Firefox, Safari, or Edge.',
      )
    }
