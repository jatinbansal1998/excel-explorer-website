...
describe('AnalyticsPanelView (presentational)', () => {
  ...
  test('shows filtered view badge when enabled and filters active', () => {
    renderWithProviders(
      <AnalyticsPanelView {...baseProps} useFilteredForLLM filtersActive filteredRowsCount={5} />,
    )
  ...
  test('tab buttons call onActiveTabChange', () => {
    ...
    renderWithProviders(<AnalyticsPanelView {...baseProps} onActiveTabChange={onActiveTabChange} />)
  ...
  test('renders suggestions and Use button calls handler', () => {
    ...
    renderWithProviders(
      <AnalyticsPanelView
        ...
        canRun
        groupedSuggestions={{
          descriptive: [{ id: '1', category: 'descriptive', prompt: 'Try X' }],
        }}
        onUseSuggestion={onUseSuggestion}
      />,
    )

    // Category header shows when selectedCategory is 'all'
