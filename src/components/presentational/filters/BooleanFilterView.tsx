  ...
  return (
    <div className="flex items-center space-x-4 text-sm">
      <label className="flex items-center space-x-2">
        <input type="radio" name={`${filter.id}-bool`} checked={val === null} onChange={() => setVal(null)} />
        <span>All</span>
      </label>
      <label className="flex items-center space-x-2">
        <input type="radio" name={`${filter.id}-bool`} checked={val === true} onChange={() => setVal(true)} />
        <span>True</span>
      </label>
      <label className="flex items-center space-x-2">
        <input type="radio" name={`${filter.id}-bool`} checked={val === false} onChange={() => setVal(false)} />
        <span>False</span>
      </label>
    </div>
  )
