...
import { PerformanceMonitor, PerformanceMetric, PerformanceSummary } from '../utils/performanceMonitor';
...
export function useInteractionPerformance() {
  ...
  const measureClick = useCallback(async (
    buttonName: string,
  ...
  const measureFormSubmit = useCallback(async (
    formName: string,
...
export function useFilePerformance() {
  ...
  const measureFileRead = useCallback(async (
    fileName: string,
    fileSize: number,
  ...
  const measureFileParse = useCallback(async (
    fileName: string,
    fileType: string,
    operation: () => Promise<any>
  ) => {
  ...
  const measureDataProcessing = useCallback(async (
...
export function usePerformanceAlerts() {
  ...
  useEffect(() => {
    ...
    summary.operations.forEach(op => {
      if (op.averageTime > 2000) { // 2 seconds
        newAlerts.push({
          type: 'slow-operation',
