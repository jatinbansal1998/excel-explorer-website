...
export class PerformanceMonitor {
  ...
  endTiming(name: string): PerformanceMetric | null {
    ...
    // Log slow operations with different thresholds
    const threshold = name.startsWith('openrouter_') ? 30000 : 1000 // 30s for LLM, 1s for others
  ...
  logMemoryUsage(_context: string): void {
    // Memory usage logging removed to reduce console noise
    // Use getMemoryUsage() method to access memory data programmatically
  }
  ...
  getSummary(): PerformanceSummary {
    ...
    for (const element of this.completedMetrics) {
    ...
    const operations = uniqueOperations.map((name) => ({
      ...
      minTime: Math.min(
        ...this.completedMetrics.filter((m) => m.name === name).map((m) => m.duration || 0),
      ),
      maxTime: Math.max(
        ...this.completedMetrics.filter((m) => m.name === name).map((m) => m.duration || 0),
      ),
