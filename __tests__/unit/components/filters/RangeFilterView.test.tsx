...
describe('RangeFilterView', () => {
  ...
  it('calls onChange with mode binned when toggling binned', async () => {
    ...
    render(
      <RangeFilterView filter={makeFilter()} onChange={onChange} columnInfo={columnInfo} filteredData={filteredData} />,
    )
    ...
    expect((last.values as any).mode).toBe('binned')
  })

  it('updates range values in continuous mode', async () => {
    const onChange = jest.fn()
    render(
      <RangeFilterView filter={makeFilter({ mode: 'continuous' })} onChange={onChange} columnInfo={columnInfo} filteredData={filteredData} />,
    )
    ...
    expect(typeof (last.values as any).currentMin).toBe('number')
  })

  it('toggles bin selection in binned mode', async () => {
    const onChange = jest.fn()
    render(
      <RangeFilterView filter={makeFilter({ mode: 'binned' })} onChange={onChange} columnInfo={columnInfo} filteredData={filteredData} />,
    )
    ...
    expect(Array.isArray((last.values as any).selectedRangeIds)).toBe(true)
  })
})
